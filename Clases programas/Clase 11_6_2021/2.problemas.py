...
def ordenar_mayor_promedio(consumos, metros, propietarios, promedio_consumo):
    "ordena a los departamentos que superan al promedio de menor a mayor"
    mayores_promedio = []

    for k in range(len(consumos)):

        if consumos[k] > promedio_consumo:
            mayores_promedio.append = consumos[k]

    # for-for-if
    for i in range(len(mayores_promedio)):
        for j in range( i+1, len(mayores_promedio)):
            if mayores_promedio[i] > mayores_promedio[j]:
                aux = mayores_promedio[j]               #hacer el swap con todos
                mayores_promedio[j] = mayores_promedio[i]
                mayores_promedio[i] = aux
